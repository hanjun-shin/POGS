...
def main(input_config)
	...
	if not input_config.has_key('modeling_parameters') :
		...
		if not input_config['modeling_parameters'].has_key('struct_file') :
			raise Exception('%s : Input config error, it does not have struct_file' % os.path.name(__file__))
	...
	output_file = input_config['modeling_parameters']['struct_file']
	actdistfile = input_config['modeling_parameters']['last_actDist']
	
	useLastCoordinates = False
	if currentfb != 'p100' :
		useLastCoordinates = True
	
    newmodel = alab.modeling.tadmodel(probfile,radNucleus=5000.0,contactRange=1)
    tstart = time.time()
...
if __name__ == "__main__":
	...
	parser.add_argument('-i', '--input_config', type=str, required=True)  #probility matrix file in contactmatrix format
